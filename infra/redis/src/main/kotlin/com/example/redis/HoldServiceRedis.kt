...
class HoldServiceRedis(
    ...
    override suspend fun get(key: ReserveKey): ReservePayload? = withContext(MDCContext() + Dispatchers.IO) {
        val bucket = redisson.getBucket<String>(key.toRedisKey())
        val json = bucket.get() ?: return@withContext null
        RedisJson.instance.decodeFromString(ReservePayload.serializer(), json)
    }

    override suspend fun exists(key: ReserveKey): Boolean = withContext(MDCContext() + Dispatchers.IO) {
        redisson.getBucket<Any>(key.toRedisKey()).isExists
    }

    override suspend fun prolong(key: ReserveKey, ttlSec: Long): Boolean = withContext(MDCContext() + Dispatchers.IO) {
        redisson.getBucket<Any>(key.toRedisKey()).expire(ttlSec, TimeUnit.SECONDS)
    }

    override suspend fun release(key: ReserveKey): Boolean = withContext(MDCContext() + Dispatchers.IO) {
